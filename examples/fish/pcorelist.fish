...
if string match -qr '^([0-3]\.|4\.0\.)' -- $version
    echo "pcorelist completion requires fish >= 4.1 (running $version)" >&2
    return 1
end
...
complete -c pcorelist -n '__pcorelist_prefix _meta completion generate' -l fish -d 'for fish (>= 4.1)'
